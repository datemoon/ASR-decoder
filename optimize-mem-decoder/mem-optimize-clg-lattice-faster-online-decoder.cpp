...
MemOptimizeClgLatticeFasterOnlineDecoder::Token *MemOptimizeClgLatticeFasterOnlineDecoder::FindOrAddToken(ClgTokenStateId stateid,
					...
					if(clgarc->_input != 0)
					{
						...
						for(int j = 0; j < static_cast<int>(hmmstate->GetArcSize()); ++j)
						{
							...
							if (arc->_input != 0)
							{
								...
								tok->_links = NewForwardLink(next_tok, arc->_input, clgarc->_output,
				...
				for(int i = 0; i < static_cast<int>(state->GetArcSize()); ++i)
				{
					...
					{ // propagate
						...
						Token *next_tok = FindOrAddToken(arrivestateid,
								frame + 1, tot_cost, tok, NULL);
						// Add ForwardLink from tok to next_tok (put on head of list tok->links)
						tok->_links = NewForwardLink(next_tok, arc->_input, arc->_output,
								graph_cost, ac_cost, tok->_links);
						_num_links++;
					}
				}
...
void MemOptimizeClgLatticeFasterOnlineDecoder::ProcessNonemitting(float cutoff)
{
	...
	for(const Elem *e = _toks.GetList(); e!= NULL; e = e->tail)
		_queue.push_back(static_cast<ClgTokenStateId>(e->key));
	...
	while (!_queue.empty())
	{
		...
		Token *tok = _toks.Find(stateid)->val; // would segfault if state not in toks_ but this can't happen.
			...
			{ // propagate eps edge
				...
				if(tot_cost < cutoff)
				{
					...
					Token *new_tok = FindOrAddToken(arrivestateid, frame, tot_cost,
							tok, &changed);
					tok->_links = NewForwardLink(new_tok, 0, arc->_output,
