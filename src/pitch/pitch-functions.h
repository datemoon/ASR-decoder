...
{
public:
	StreamPitch(const PitchExtractionOptions &pitch_opts,
			const ProcessPitchOptions &process_opts):prev_frame_offset_(0),
		pitch_extractor_(pitch_opts),post_process_(process_opts, &pitch_extractor_)
	{
	...
	int32 GetDim() { return post_process_.Dim();}
	~StreamPitch() { }
	void ProcessWave(PitchExtractionOptions &pitch_opts,
			Vector<BaseFloat> &wavform, Matrix<BaseFloat> &features, bool is_end)
	{
		int32 cur_rows = 10;
		Matrix<BaseFloat> feats(cur_rows,post_process_.Dim(), kSetZero);
		pitch_extractor_.AcceptWaveform(pitch_opts.samp_freq, wavform);
		if(is_end == true)
			pitch_extractor_.InputFinished();
		int32 cur_frame = 0;
//		std::cout << post_process_.NumFramesReady() << std::endl;
		for(cur_frame = prev_frame_offset_; cur_frame < post_process_.NumFramesReady();
				++cur_frame)
		{
			if(cur_frame - prev_frame_offset_ >= cur_rows)
			{
				cur_rows *= 2;
				feats.Resize(cur_rows, post_process_.Dim(), kCopyData);
			}
			SubVector<BaseFloat> row(feats, cur_frame-prev_frame_offset_);
			post_process_.GetFrame(cur_frame, &row);
		}

		if(cur_frame-prev_frame_offset_ != 0)
			features = feats.RowRange(0, cur_frame-prev_frame_offset_);
		prev_frame_offset_ = cur_frame;
		if(is_end == true)
			prev_frame_offset_ = 0;
	}

	void ProcessWave(PitchExtractionOptions &pitch_opts,
			short *wav_data,int32 wav_len,
			BaseFloat **feature,int32 &dim, int32 &rows, bool is_end)
	{
		...
		ProcessWave1(pitch_opts,wavform,features, is_end);
//		std::cout << "*********" << std::endl;
//		features.Print();
		// there make sure features is continue storage.
		dim = features.NumCols();
		rows = features.NumRows();
		*feature = NULL;
		if(rows != 0)
		{
			*feature = new float[dim * rows];
			memcpy(*feature, features.Data(), dim * rows * sizeof(float));
		}
	}
	
	void ProcessWave1(PitchExtractionOptions &pitch_opts,
			Vector<BaseFloat> &wavform, Matrix<BaseFloat> &features, bool is_end)
	{
		int32 cur_offset = 0, cur_frame = 0;
		int32 samp_per_chunk = pitch_opts.frames_per_chunk *
			pitch_opts.samp_freq * pitch_opts.frame_shift_ms / 1000.0f;

		while(cur_offset < wavform.Dim())
		{
			int32 num_samp;
			if (samp_per_chunk > 0)
				num_samp = std::min(samp_per_chunk, wavform.Dim() - cur_offset);
			else  // user left opts.frames_per_chunk at zero.
				num_samp = wavform.Dim();
			SubVector<BaseFloat> wave_chunk(wavform, cur_offset, num_samp);
			pitch_extractor_.AcceptWaveform(pitch_opts.samp_freq, wave_chunk);
			cur_offset += num_samp;
			if(cur_offset == wavform.Dim() && is_end == true)
				pitch_extractor_.InputFinished();
...
private:
	int32 prev_frame_offset_;
	OnlinePitchFeature pitch_extractor_;
	OnlineProcessPitch post_process_;
};
