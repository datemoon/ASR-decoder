...
void ComputeNccf(const VectorBase<BaseFloat> &inner_prod,
  ...
  assert(inner_prod.Dim() == norm_prod.Dim() &&
...
void ComputeLocalCost(const VectorBase<BaseFloat> &nccf_pitch,
  ...
  assert(nccf_pitch.Dim() == local_cost->Dim() &&
...
void PitchFrameInfo::SetNccfPov(const VectorBase<BaseFloat> &nccf_pov) {
  ...
  assert(num_states == (int)state_info_.size());
...
void PitchFrameInfo::Cleanup(PitchFrameInfo *prev_frame) {
	std::cerr << "Cleanup not implemented." << std::endl;
}
...
void OnlinePitchFeatureImpl::UpdateRemainder(
  ...
  if (next_frame_sample > next_downsampled_samples_processed) {
    ...
    assert(full_frame_length < frame_shift && "Code error");
...
void OnlinePitchFeatureImpl::GetFrame(int32 frame,
  ...
  assert(frame < NumFramesReady() && feat->Dim() == 2);
...
void OnlinePitchFeatureImpl::InputFinished() {
  ...
  frames_latency_ = 0;
#ifdef DEBUG
  std::cout << "Pitch-tracking Viterbi cost is "
                << (forward_cost_remainder_ / num_frames)
                << " per frame, over " << num_frames << " frames." << std::endl;
#endif
}
...
#ifdef DEBUG
  std::cout << "Forward-cost per frame changed from "
                << (forward_cost_remainder_ / num_frames) << " to "
                << (forward_cost_remainder / num_frames) << std::endl;
#endif
...
void OnlinePitchFeatureImpl::AcceptWaveform(
  ...
  frames_latency_ =
      frame_info_.back()->ComputeLatency(opts_.max_frames_latency);
#ifdef DEBUG
  std::cout << "Latency is " << frames_latency_ << std::endl;
#endif
}
...
void ComputeKaldiPitchFirstPass(
  ...
  if (cur_frame  == 0) {
	std::cout << "No features output since wave file too short" << std::endl;
    output->Resize(0, 0);
  } else {
...
void ComputeKaldiPitch(const PitchExtractionOptions &opts,
  ...
  if (num_frames == 0) {
	std::cout << "No frames output in pitch extraction" << std::endl;
    output->Resize(0, 0);
    return;
  }
...
//#define COUHE
#ifdef COUHE
  // this code don't make sense,it's only for uniformity
  srand(1);
  for(unsigned i=0;i<delta_feature_noise_.size();++i)
  {
	  RandGauss();
  }
#endif
