...
void ArbitraryResample::Resample(const MatrixBase<BaseFloat> &input,
  ...
  assert(input.NumRows() == output->NumRows() &&
               input.NumCols() == num_samples_in_ &&
               output->NumCols() == (int)(weights_.size()));
...
void ArbitraryResample::Resample(const VectorBase<BaseFloat> &input,
  ...
  assert(input.Dim() == num_samples_in_ &&
               output->Dim() == (int)(weights_.size()));
...
template<class I> I  Gcd(I m, I n)
{
	if (m == 0 || n == 0) {
		if (m == 0 && n == 0) {  // gcd not defined, as all integers are divisors.
			std::cerr << "Undefined GCD since m = 0, n = 0." << std::endl;
		}
