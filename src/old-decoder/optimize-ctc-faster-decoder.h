...
struct CtcFasterDecoderOptions 
{
  	...
  	CtcFasterDecoderOptions(): 
		...
		_beam_delta(0.5),
		_hash_ratio(2.0) { }
	CtcFasterDecoderOptions(float beam,int max_active,
	...
	void Register(ConfigParseOptions *opts, bool full)
	{  /// if "full", use obscure
		/// options too.
		/// Depends on program.
...
{
public:
  	CtcFasterDecoder(Fst *graph,
			const CtcFasterDecoderOptions *config);

  	void SetOptions(const CtcFasterDecoderOptions *config) { _config = config; }

  	~CtcFasterDecoder() { ClearToks(); }
  	...
  	/// no tokens survived).
  	bool GetBestPath(vector<int> &words_arr , vector<int> &phone_arr, 
...
*/
	...
	// It actually allows us to maintain
	...
	unordered_map<StateId, Token*> _prev_toks;
  	...
  	Fst *_graph;
  	const CtcFasterDecoderOptions *_config;
