...
void MergeSameAduio(const VadSeg &vad_ali,
		VadSeg &merge_vad_ali)
{
	merge_vad_ali.clear();
	if(vad_ali.size() == 0)
		return;
	merge_vad_ali.push_back(vad_ali[0]);
	for(size_t i = 1; i < vad_ali.size(); ++i)
	{
		if(merge_vad_ali.back().first == vad_ali[i].first)
		{
			merge_vad_ali.back().second.second = vad_ali[i].second.second;
			continue;
		}
		merge_vad_ali.push_back(vad_ali[i]);
	}
}

void CompressAlignVadAndRestrictMaxNosilFrame(const VadSeg &vad_ali_ori,
		VadSeg &compress_vad_ali_last, int32 sil_frames=50, 
		int32 max_nosil_frames=-1, int32 min_nosil_frames=0)
{
	compress_vad_ali_last.clear();
	if(max_nosil_frames <= 0)
	{
		CompressAlignVad(vad_ali_ori, compress_vad_ali_last, sil_frames);
		return;
	}
	VadSeg vad_ali, compress_vad_ali;
	// first merge adjoin same audio
	MergeSameAduio(vad_ali_ori, vad_ali);
	for(size_t i = 0 ; i < vad_ali.size() ; i++)
	{
		if(vad_ali[i].first == SIL)
		{
		...
		else if(vad_ali[i].first == AUDIO)
		{
			...
			{ // 现在已经是中间的语音段
				KALDI_ASSERT(compress_vad_ali.back().first == SIL);
				//int32 cur_len = vad_ali[i].second.second - vad_ali[i].second.first;
				int32 prev_sil_len = compress_vad_ali.back().second.second - compress_vad_ali.back().second.first;
				size_t id = compress_vad_ali.size()-2;
				//int32 prev_nosil_len = compress_vad_ali[id].second.second -  compress_vad_ali[id].second.first;
				if(prev_sil_len < sil_frames && 
						vad_ali[i].second.second - compress_vad_ali[id].second.first < max_nosil_frames)
				{ // 删掉这段静音，将其和前后2段语音合成一段语音
					compress_vad_ali.pop_back();
					compress_vad_ali.back().second.second = vad_ali[i].second.second;
				}
	...
	// cut greater than max_nosil_frames nosil and delete too short nosil
	for(size_t i = 0 ; i<compress_vad_ali.size(); ++i)
	{
		if(compress_vad_ali[i].first == SIL)
		{
			compress_vad_ali_last.push_back(compress_vad_ali[i]);
			continue;
		}
		else if(compress_vad_ali[i].first == AUDIO)
		{
			int32 cur_len = compress_vad_ali[i].second.second - compress_vad_ali[i].second.first;
			if(cur_len > max_nosil_frames)
			{
				int32 start_frames = compress_vad_ali[i].second.first;
				while(cur_len > 0)
				{// too length
					int32 end_frames = start_frames + max_nosil_frames;
					if(end_frames > compress_vad_ali[i].second.second)
						end_frames = compress_vad_ali[i].second.second;
					if(min_nosil_frames > end_frames-start_frames)
					{
						compress_vad_ali_last.push_back(std::make_pair(SIL,
									std::make_pair(start_frames, end_frames)));
					}
					else
					{
						compress_vad_ali_last.push_back(std::make_pair(compress_vad_ali[i].first,
									std::make_pair(start_frames, end_frames)));
						// insert sil
						compress_vad_ali_last.push_back(std::make_pair(SIL,
									std::make_pair(end_frames, end_frames)));
					}
					cur_len -= max_nosil_frames;
					start_frames += max_nosil_frames;
				}
			}
			else if(cur_len < min_nosil_frames)
			{ // too short
				compress_vad_ali_last.push_back(std::make_pair(SIL,
							compress_vad_ali[i].second));
			}
			else
			{
				compress_vad_ali_last.push_back(compress_vad_ali[i]);
...
public:
			...
			int32 max_nosil_frames=-1, int32 min_nosil_frames=0)
	{
		...
		{
			CompressAlignVadAndRestrictMaxNosilFrame(*vad_ali_tmp, vad_ali, 
					min_sil_frames_interval, max_nosil_frames, min_nosil_frames);
			sil_frames = 0;
			nosil_frames = 0;
			for(size_t i=0 ; i < vad_ali.size(); ++i)
			{
...
{
	int32 _samp_freq;
	BaseFloat _frame_length_s;
	BaseFloat _frame_shift_s;
	BaseFloat _energy_threshold1;
	BaseFloat _energy_threshold2;
	std::string _cal_method;
	std::string _prefix;

	EnergyVadOpts(std::string prefix="energy-vad"):
		_samp_freq(16000),
		_frame_length_s(0.025),
		_frame_shift_s(0.01),
		_energy_threshold1(32768*0.005),
		_energy_threshold2(32768*0.05),
		_cal_method("sum_square_root"),
		_prefix(prefix) { }
	...
	void Register(OptionsItf *opts)
	{
		// register the vad judge options with the prefix "vad-judge".
		ParseOptions vad_opts(_prefix, opts);
		vad_opts.Register("samp-freq", &_samp_freq,
				"energy vad calculate option, sample frequence.");
		vad_opts.Register("frame-length-s", &_frame_length_s,
				"energy vad calculate option, frame length sec");
		vad_opts.Register("frame-shift-s", &_frame_shift_s,
				"energy vad calculate option, frame shift sec");
		vad_opts.Register("energy-threshold1", &_energy_threshold1,
				"energy vad calculate option, energy threshold1, if greater then set 1");
		vad_opts.Register("energy-threshold2", &_energy_threshold2,
				"energy vad calculate option, energy threshold2, if greater then set 2");
		vad_opts.Register("cal-method", &_cal_method,
				"energy vad calculate option, energy calculate method.");
	}
};
...
{
public:
	V1EnergyVad(const VadJudgeOptions &vad_judge_opt,
		...
		_vad_judge_opt(vad_judge_opt),
		_vad_judge(vad_judge_opt)
	{
	...
	V1EnergyVad(const VadJudgeOptions &vad_judge_opt,
			const EnergyVadOpts &energy_vad_opt,
			int32 vad_frame_offset = 0):
		_cal_method(energy_vad_opt._cal_method),
		_samp_freq(energy_vad_opt._samp_freq),
		_frame_length_s(energy_vad_opt._frame_length_s),
		_frame_shift_s(energy_vad_opt._frame_shift_s),
		_vad_frame_offset(vad_frame_offset),
		_energy_threshold1(energy_vad_opt._energy_threshold1),
		_energy_threshold2(energy_vad_opt._energy_threshold2),
		_vad_judge_opt(vad_judge_opt),
		_vad_judge(vad_judge_opt) 
	{ 
		_frame_length_samp = _samp_freq * _frame_length_s;
...
public:
			...
			int32 max_nosil_frames=-1, int32 min_nosil_frames=0)
	{
		...
		if(compress_vad_ali == true)
		{
			CompressAlignVadAndRestrictMaxNosilFrame(*vad_ali_tmp, vad_ali, min_sil_frames_interval, 
					max_nosil_frames, min_nosil_frames);
			sil_frames = 0;
			nosil_frames = 0;
			for(size_t i=0 ; i < vad_ali.size(); ++i)
			{
...
private:
	...
	// vad context
	const VadJudgeOptions &_vad_judge_opt;
	VadJudge _vad_judge;
};

struct VadProcessOpts
{
	VadJudgeOptions _energy_vad_judge_opt;
	EnergyVadOpts _energy_vad_opt;
	bool _use_realtime_vad;
	bool _print_vad_ali_info;
	int32 _min_sil_frames_interval;
	int32 _max_nosil_frames;
	int32 _min_nosil_frames;

	VadProcessOpts(std::string vad_judge_prefix= "energy-vad-judge",
		   std::string energy_vad_prefix = "energy-vad"):
		_energy_vad_judge_opt(vad_judge_prefix),
		_energy_vad_opt(energy_vad_prefix),
 		_use_realtime_vad(true),
		_print_vad_ali_info(false),
		_min_sil_frames_interval(50),
 		_max_nosil_frames(1000),
		_min_nosil_frames(0) { }

	void Register(kaldi::OptionsItf *opts)
	{
		_energy_vad_judge_opt.Register(opts);
		_energy_vad_opt.Register(opts);
		opts->Register("use-realtime-vad", &_use_realtime_vad,
				"Use realtime vad or not.");
		opts->Register("print-vad-ali-info", &_print_vad_ali_info,
				"Print vad info.");
		opts->Register("min-sil-frames-interval", &_min_sil_frames_interval,
				"Vad min silence frames interval.");
		opts->Register("max-nosil-frames", &_max_nosil_frames,
			"max no silence frames number. if max-nosil-frames <= 0 , will not be restrict max nosil frames.");
		opts->Register("min-nosil-frames", &_min_nosil_frames,
			"min no silence frames number. if min-nosil-frames > 0 , will not be restrict min nosil frames.");
	}
};

// 
template<class T>
class EnergyVadProcess
{
public:
	EnergyVadProcess(const VadProcessOpts &vad_process_opt):
		_vad_process_opt(vad_process_opt),
		_v1_energy_vad(_vad_process_opt._energy_vad_judge_opt, _vad_process_opt._energy_vad_opt, 0),
	   	_tot_sil_frames(0),
	   	_tot_nosil_frames(0),  _pre_energy_seg_flag(SIL) { }

	void PushData(VectorBase<T> &indata)
	{
		_v1_energy_vad.AcceptData(indata);
	}

	// input:
	//     indata   : 返回的非语音段数据
	//     end_flag : 结束标志
	//     data_start_frame: 记录每段语音的开始时间，以帧为单位
	// return:
	//     如果最后一个是一整段为true，否则为false
	bool GetData(std::vector<std::vector<T> > &indata, bool end_flag = false, 
			std::vector<int> *data_start_frame = NULL)
	{
		if(data_start_frame != NULL)
			data_start_frame->clear();
		indata.clear();
		VadSeg vad_ali = _v1_energy_vad.ComputerVad(end_flag);
		if(_vad_process_opt._use_realtime_vad != true)
		{ // not real time
			if(true == end_flag)
			{ // 压缩vad的切割，将非静音间静音段小于_v1_asr_opts._min_sil_frames_interval的静音和前后非静音合并为一段非静音
				...
				_v1_energy_vad.GetTotalVadAli(vad_ali, sil_frames, nosil_frames,
						true, _vad_process_opt._min_sil_frames_interval, 
						_vad_process_opt._max_nosil_frames,
						_vad_process_opt._min_nosil_frames);
				PrintVadSeg(vad_ali, "compress-energy-vad-ali-> ", 3);
			}
			else
				return false;
		}
		std::vector<BaseFloat> nosil_wav_data;
		int32 true_start = 0;
				...
				it != vad_ali.end(); ++it)
		{
			...
			if(it->first == SIL)
			{
				...
				if(_pre_energy_seg_flag == AUDIO)
				{
					...
					nosil_wav_data.insert(nosil_wav_data.end(), cur_wav_data.begin(), cur_wav_data.end());
					indata.push_back(std::move(nosil_wav_data));
					if(data_start_frame != NULL)
						data_start_frame->push_back(true_start);
					nosil_wav_data.clear();
				}
			...
			else if(it->first == AUDIO)
			{
				...
				if(it+1 == vad_ali.end() && end_flag == true)
				{// 如果是当前最后一段并且是全局结束
					_v1_energy_vad.GetData(nosil_wav_data, start_frame, end_frame, true);
					indata.push_back(std::move(nosil_wav_data));
					if(data_start_frame != NULL)
						data_start_frame->push_back(true_start);
					nosil_wav_data.clear();
				...
				{
					_v1_energy_vad.GetData(nosil_wav_data, start_frame, end_frame, false);
					if(it+1 == vad_ali.end())
					{ // 如果是当前最后一段并且，如果不是最后一段留到下一段静音段放入indata.
						indata.push_back(std::move(nosil_wav_data));
						if(data_start_frame != NULL)
							data_start_frame->push_back(true_start);
						nosil_wav_data.clear();
		...
		if(_pre_energy_seg_flag == SIL || end_flag == true)
			return true;
		else
			return false;
	}
	...
	void GetInfo(int32 &sil_frames, int32 &nosil_frames)
	{
		sil_frames = _tot_sil_frames;
		nosil_frames = _tot_nosil_frames;

		if(_vad_process_opt._print_vad_ali_info)
		{
			VadSeg vad_ali;
			int32 sil_frames = -1, nosil_frames = -1;
			if(_vad_process_opt._use_realtime_vad != true)
			...
			KALDI_VLOG(0) << "sil_frames is : " << sil_frames 
			   << "\tnosil_frames is : " << nosil_frames;
			PrintVadSeg(vad_ali, "compress-energy-vad-ali-> ", 0);
		}
	}
private:
	const VadProcessOpts &_vad_process_opt;
	V1EnergyVad<T> _v1_energy_vad;
