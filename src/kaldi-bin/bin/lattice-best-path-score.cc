...
int main(int argc, char *argv[]) {
  try {
	...
	std::string best_score_filename;
	...
	po.Register("best-score-filename", &best_score_filename, "output best score filename");
	...
	if(best_score_filename.empty())
	{
		best_score_filename = "ark,t:score.txt";
	}
	TableWriter<BasicVectorHolder<BaseFloat> > best_score_writer(best_score_filename);
    ...
    for (; !clat_reader.Done(); clat_reader.Next()) {
      ...
      if (best_path.Start() == fst::kNoStateId) {
		...
		std::vector<BaseFloat> best_score = {weight.Value1(), weight.Value2(), (weight.Value1() + weight.Value2())};
		best_score_writer.Write(key, best_score);
