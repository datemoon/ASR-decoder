...
void LstmCudnn::InitBuf()
{
	_buffer = new float[2 * _dim * 3]; // c h m
	memset(_buffer,0x00,sizeof(float)*2 * _dim * 3);
}

void LstmCudnn::MallocGifoMemory(int frames)
{
...
void LstmCudnn::PropagateFnc(float *in, int frames, int cols, float *out )
{
...
//	PrintPoint(out,frames,_dim);
}

bool LstmCudnn::ReadData(FILE *fp,bool binary)
{
	...
	_bias = new float[_rows * CUDNN_BIAS];
...
//	_phole_o_c = new float[_rows];
	...
	// read _bias
	ReadMatrix(fp, _bias , 1, _rows * CUDNN_BIAS);
