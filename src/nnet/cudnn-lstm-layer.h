...
public:
#define CUDNN_BIAS 2
	LstmCudnn(int input_dim,int output_dim):
...
//		_phole_f_c(NULL),_phole_o_c(NULL),
		_dim(0) { }
...
//		if(_phole_o_c != NULL) delete [] _phole_o_c;
	...
	ComponentType GetType() const { return kLstmCudnn;}

	void GetType(string &type) const { type = "LstmCudnn";}
...
//		_phole_o_c = new float[_rows];
		...
		if(_rows * CUDNN_BIAS != (int)fread(_bias,sizeof(float),_rows * CUDNN_BIAS,fp))
			return false;
...
//			return false;
	...
	bool WriteData(FILE *fp)
	{
		...
		if(_rows * CUDNN_BIAS != (int)fwrite(_bias,sizeof(float),_rows * CUDNN_BIAS,fp))
			return false;
