...
void FasterDecoder::InitDecoding()
{
  	...
  	StdArc dummy_arc(0,0,0,start_state);
...
{
  	InitDecoding();
  	while (!decodable->IsLastFrame(_num_frames_decoded - 1))
   	{
  	...
  	if (best_tok)
   	{
		...
		Token *tok = best_tok;
		StdNode *state = (StdNode *)_graph->GetNode((unsigned)stateid);
