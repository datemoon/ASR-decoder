...
LatticeFasterDecoder::Elem *LatticeFasterDecoder::FindOrAddToken(StateId stateid,
 ...
 *************/
float LatticeFasterDecoder::GetCutoff(Elem *list_head, size_t *tok_count,
		...
		{// it's OK.
			...
			for(int i = 0; i < static_cast<int>(state->GetArcSize()); ++i)
			{
				...
				{ // propagate
					...
					tok->_links = new ForwardLink(next_tok->val, arc->_input, arc->_output,
			...
			{ // propagate eps edge
				...
				if(tot_cost < cutoff)
				{
					...
					Elem *new_tok = FindOrAddToken(arc->_to, frame, tot_cost, tok,
							&changed);
					tok->_links = new ForwardLink(new_tok->val, 0, arc->_output,
...
bool LatticeFasterDecoder::GetBestPath(Lattice *ofst, 
		bool use_final_probs) const
{
	ofst->DeleteStates();
